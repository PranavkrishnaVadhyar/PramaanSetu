    ...
    <div className="min-h-screen bg-bg flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-surface border border-border rounded-lg shadow-sm p-8">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-text-secondary mt-2">Get your API keys and access the sandbox</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-medium text-text-secondary mb-1">Organization Name <span className="text-stone-400 font-normal">(Optional)</span></label>
            <input
              type="text"
              value={organization}
        ...
        <div className="mt-6 text-center text-xs text-text-secondary">
          Already have an account? <Link to="/login" className="text-stone-900 dark:text-stone-100 font-medium hover:underline">Log in</Link>
        </div>
      </div>
    </div>
  );
